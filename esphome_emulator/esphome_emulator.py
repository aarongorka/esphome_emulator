...
type GetListsResponse = list[api.ListEntitiesBinarySensorResponse | api.ListEntitiesTextSensorResponse | api.ListEntitiesSensorResponse]

type GetStatesResponse = list[api.SensorStateResponse | api.TextSensorStateResponse | api.BinarySensorStateResponse]
...
def handle_request(client_socket, message_map: dict[int, str], get_lists: Callable[[], GetListsResponse], get_states: Callable[[], GetStatesResponse]):
    ...
    if message_type_name == "HelloRequest":
        ...
        request.ParseFromString(data)
        print(f"Parsed {request.DESCRIPTOR.name}: {str(request).strip()}")
        raise Exception("This script can't handle multiple clients yet, aborting...")
        ...
        # response = api.SubscribeHomeAssistantStateResponse()
        # response.entity_id = "zone.home"
        # response.attribute = "Persons"
        # encoded_response = encode_message(response)
        # print(f"Sending {response.DESCRIPTOR.name}: {encoded_response}...")
        # client_socket.sendall(encoded_response)
        # print(f"Sent {response.DESCRIPTOR.name}.")
...
def run(get_lists: Callable[[], GetListsResponse], get_states: Callable[[], GetStatesResponse]):
    ...
    try:
        ...
        while True:
            handle_request(client_socket, message_map, get_lists, get_states)
